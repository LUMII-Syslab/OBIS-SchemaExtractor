...
public enum SchemaExtractorQueries {
    ...
    FIND_INSTANCE_COUNT_FOR_CLASS(
            "SELECT (COUNT(?x) as ?instances) WHERE { ?x <classificationProperty> <classA>. }"
    ),
    ...
    FIND_TRIPLE_COUNT_FOR_PROPERTY(
            "SELECT (COUNT(?x) as ?instances) WHERE { ?x <property> ?y. }"
    ),
    FIND_TRIPLE_COUNT_FOR_PROPERTY_DISTINCT(
            "SELECT (COUNT(*) as ?instances) WHERE {{SELECT DISTINCT ?x ?y WHERE { ?x <property> ?y. }}}"
    ),
    ...
    FIND_PROPERTY_DATA_TYPE_WITH_TRIPLE_COUNT_DISTINCT(
            "SELECT ?dataType (COUNT(?value) as ?instances) WHERE { { SELECT DISTINCT ?x ?value ?dataType WHERE {?x <property> ?value. BIND (datatype(?value) as ?dataType). FILTER (BOUND(?dataType)) }}} GROUP BY ?dataType"
    ),
    ...
    FIND_PROPERTY_DATA_TYPE_WITH_TRIPLE_COUNT_WITH_LIMITS_DISTINCT(
            "SELECT ?dataType (COUNT(?value) as ?instances) WHERE { {SELECT DISTINCT ?x ?value ?dataType WHERE {?x <property> ?value. BIND (datatype(?value) as ?dataType)} LIMIT <limit> }} GROUP BY ?dataType"
    ),
    ...
    FIND_PROPERTY_DATA_TYPE_WITH_TRIPLE_COUNT_FOR_SOURCE_DISTINCT(
            "SELECT ?dataType (COUNT(?value) as ?instances) WHERE { {SELECT DISTINCT ?x ?value ?dataType WHERE { ?x <classificationProperty> <sourceClass>. ?x <property> ?value. BIND (datatype(?value) as ?dataType). FILTER (BOUND(?dataType)) }}} GROUP BY ?dataType"
    ),
    ...
    FIND_PROPERTY_DATA_TYPE_WITH_TRIPLE_COUNT_FOR_SOURCE_WITH_LIMITS_DISTINCT(
            "SELECT ?dataType (COUNT(?value) as ?instances) WHERE { { SELECT DISTINCT ?x ?value ?dataType WHERE {?x <classificationProperty> <sourceClass>. ?x <property> ?value. BIND (datatype(?value) as ?dataType)} LIMIT <limit> }} GROUP BY ?dataType"
    ),
    ...
    FIND_PROPERTY_DATA_TYPE_LANG_STRING_DISTINCT(
            "SELECT (COUNT(?value) as ?instances) WHERE { { SELECT DISTINCT ?x ?value WHERE { ?x <property> ?value. FILTER (lang(?value) != \"\") }} }"
    ),
    ...
    FIND_PROPERTY_DATA_TYPE_LANG_STRING_WITH_LIMITS_DISTINCT(
            "SELECT (COUNT(?value) as ?instances) WHERE { { SELECT DISTINCT ?x ?value WHERE {?x <property> ?value.} LIMIT <limit> } FILTER (lang(?value) != \\\"\\\") }"
    ),
    ...
    FIND_PROPERTY_DATA_TYPE_LANG_STRING_FOR_SOURCE_DISTINCT(
            "SELECT (COUNT(?value) as ?instances) WHERE { { SELECT DISTINCT ?x ?value WHERE { ?x <classificationProperty> <sourceClass>. ?x <property> ?value. FILTER (lang(?value) != \"\") }}}"
    ),
    ...
    FIND_PROPERTY_DATA_TYPE_LANG_STRING_FOR_SOURCE_WITH_LIMITS_DISTINCT(
            "SELECT (COUNT(?value) as ?instances) WHERE { { SELECT DISTINCT ?x ?value WHERE { ?x <classificationProperty> <sourceClass>. ?x <property> ?value. } LIMIT <limit> } FILTER (lang(?value) != \"\") }"
    ),
    ...
    FIND_PROPERTY_SOURCES_WITH_TRIPLE_COUNT(
            "SELECT ?class (COUNT(?x) as ?instances) WHERE {?x <property> ?y. ?x <classificationProperty> ?class. } GROUP BY ?class"
    ),
    FIND_PROPERTY_SOURCES_WITH_TRIPLE_COUNT_DISTINCT(
            "SELECT ?class (COUNT(?x) as ?instances) WHERE { { SELECT DISTINCT ?x ?y ?class WHERE { ?x <property> ?y. ?x <classificationProperty> ?class. }}} GROUP BY ?class"
    ),
    ...
    FIND_PROPERTY_SOURCE_TRIPLE_COUNT_DISTINCT(
            "SELECT (COUNT(?x) as ?instances) WHERE { { SELECT DISTINCT ?x ?y WHERE {?x <property> ?y. ?x <classificationProperty> <sourceClass>. }}}"
    ),
    FIND_PROPERTY_SOURCE_TRIPLE_COUNT_WITH_LIMITS(
            "SELECT (COUNT(?x) as ?instances) WHERE { { SELECT ?x WHERE {?x <classificationProperty> <sourceClass>. } LIMIT <limit> } ?x <property> ?y. }"
    ),
    FIND_PROPERTY_SOURCE_TRIPLE_COUNT_WITH_LIMITS_DISTINCT(
            "SELECT (COUNT(?x) as ?instances) WHERE { {SELECT DISTINCT ?x ?y WHERE {{ SELECT DISTINCT ?x WHERE {?x <classificationProperty> <sourceClass>. } LIMIT <limit> } ?x <property> ?y. }}}"
    ),
    ...
    FIND_PROPERTY_TARGETS_WITH_TRIPLE_COUNT(
            "SELECT ?class (COUNT(?x) as ?instances) WHERE {?x <property> ?y. ?y <classificationProperty> ?class. } GROUP BY ?class"
    ),
    FIND_PROPERTY_TARGETS_WITH_TRIPLE_COUNT_DISTINCT(
            "SELECT ?class (COUNT(?y) as ?instances) WHERE { {SELECT DISTINCT ?x ?y ?class WHERE {?x <property> ?y. ?y <classificationProperty> ?class. }}} GROUP BY ?class"
    ),
    ...
    FIND_PROPERTY_TARGET_TRIPLE_COUNT_DISTINCT(
            "SELECT (COUNT(?x) as ?instances) WHERE { {SELECT DISTINCT ?x <property> ?y. ?y <classificationProperty> <targetClass>. }"
    ),
    FIND_PROPERTY_TARGET_TRIPLE_COUNT_WITH_LIMITS(
            "SELECT (COUNT(?x) as ?instances) WHERE { { SELECT ?y WHERE {?y <classificationProperty> <targetClass>. } LIMIT <limit> } ?x <property> ?y. }"
    ),
    FIND_PROPERTY_TARGET_TRIPLE_COUNT_WITH_LIMITS_DISTINCT(
            "SELECT (COUNT(?x) as ?instances) WHERE { SELECT DISTINCT ?x ?y WHERE {{ SELECT DISTINCT ?y WHERE {?y <classificationProperty> <targetClass>. } LIMIT <limit> } ?x <property> ?y. }}}"
    ),
    ...
    FIND_PROPERTY_SOURCE_TARGET_PAIRS_DISTINCT(
            "SELECT ?sourceClass ?targetClass (COUNT(?x) as ?instances) WHERE { { SELECT DISTINCT ?x ?y ?sourceClass ?targetClass WHERE {?x <property> ?y. ?x <classificationPropertySource> ?sourceClass. ?y <classificationPropertyTarget> ?targetClass. }}} GROUP BY ?sourceClass ?targetClass"
    ),
    ...
    FIND_PROPERTY_SOURCE_TARGET_PAIRS_FOR_SPECIFIC_CLASSES_DISTINCT(
            "SELECT (COUNT(?x) as ?instances) WHERE { {SELECT DISTINCT ?x ?y WHERE {?x <property> ?y. ?x <classificationPropertySource> <sourceClass>. ?y <classificationPropertyTarget> <targetClass>.}}}"
    ),

    COUNT_PROPERTY_URL_VALUES(
            "SELECT (COUNT(?x) as ?instances) WHERE {?x <property> ?y. FILTER(!isLiteral(?y)) }"
    ),
    COUNT_PROPERTY_URL_VALUES_DISTINCT(
            "SELECT (COUNT(?x) as ?instances) WHERE { { SELECT DISTINCT ?x ?y WHERE {?x <property> ?y. FILTER(!isLiteral(?y)) }}}"
    ),
    CHECK_PROPERTY_URL_VALUES(
            "SELECT ?y where {?x <property> ?y. FILTER(!isLiteral(?y)) } LIMIT 1"
    ),
    FIND_PROPERTY_URL_VALUES_FOR_SOURCES(
            "SELECT ?class (COUNT(?x) as ?instances) WHERE {?x <property> ?y. ?x <classificationProperty> ?class. FILTER(!isLiteral(?y))} GROUP BY ?class"
    ),
    FIND_PROPERTY_URL_VALUES_FOR_SOURCES_DISTINCT(
            "SELECT ?class (COUNT(?x) as ?instances) WHERE { {SELECT DISTINCT ?x ?y ?class WHERE {?x <property> ?y. ?x <classificationProperty> ?class. FILTER(!isLiteral(?y))}}} GROUP BY ?class"
    ),
    COUNT_PROPERTY_URL_VALUES_FOR_SOURCE(
            "SELECT (COUNT(?x) as ?instances) WHERE {?x <classificationProperty> <sourceClass>. ?x <property> ?y. FILTER(!isLiteral(?y)) }"
    ),
    COUNT_PROPERTY_URL_VALUES_FOR_SOURCE_DISTINCT(
            "SELECT (COUNT(?x) as ?instances) WHERE { {SELECT DISTINCT ?x ?y WHERE {?x <classificationProperty> <sourceClass>. ?x <property> ?y. FILTER(!isLiteral(?y)) }}"
    ),
    ...
    FIND_INSTANCE_NAMESPACES_FOR_CLASS(
            "SELECT ?nspace (COUNT(?subject) AS ?nsCount) WHERE { ?subject <classificationProperty> <classA>. FILTER (isIRI(?subject)) BIND(REPLACE(STR(?subject), \"(.*[/#]).*\", \"$1\") AS ?nspace) } GROUP BY ?nspace ORDER BY DESC(?nsCount)"
    ),
    FIND_INSTANCE_NAMESPACES_FOR_CLASS_WITH_LIMIT(
            "SELECT ?nspace (COUNT(?subject) AS ?nsCount) WHERE { { SELECT ?subject WHERE { ?subject <classificationProperty> <classA>. FILTER (isIRI(?subject)) } LIMIT <limit> } BIND(REPLACE(STR(?subject), \"(.*[/#]).*\", \"$1\") AS ?nspace) } GROUP BY ?nspace ORDER BY DESC(?nsCount)"
    ),

    FIND_INSTANCE_NAMESPACES_FOR_PROPERTY_SUBJECTS(
            "SELECT ?nspace (COUNT(?subject) AS ?nsCount) WHERE { ?subject <property> ?object . FILTER (isIRI(?subject)) BIND(REPLACE(STR(?subject), \"(.*[/#]).*\", \"$1\") AS ?nspace)} GROUP BY ?nspace ORDER BY DESC(?nsCount)"
    ),
    FIND_INSTANCE_NAMESPACES_FOR_PROPERTY_SUBJECTS_WITH_LIMIT(
            "SELECT ?nspace (COUNT(?subject) AS ?nsCount) WHERE { { SELECT ?subject WHERE { ?subject <property> ?object . FILTER (isIRI(?subject)) } LIMIT <limit> } BIND(REPLACE(STR(?subject), \"(.*[/#]).*\", \"$1\") AS ?nspace)} GROUP BY ?nspace ORDER BY DESC(?nsCount)"
    ),

    FIND_INSTANCE_NAMESPACES_FOR_PROPERTY_OBJECTS(
            "SELECT ?nspace (COUNT(?object) AS ?nsCount) WHERE { ?subject <property> ?object . FILTER (isIRI(?object)) BIND(REPLACE(STR(?object), \"(.*[/#]).*\", \"$1\") AS ?nspace)} GROUP BY ?nspace ORDER BY DESC(?nsCount)"
    ),
    FIND_INSTANCE_NAMESPACES_FOR_PROPERTY_OBJECTS_WITH_LIMIT(
            "SELECT ?nspace (COUNT(?object) AS ?nsCount) WHERE { { SELECT ?object WHERE { ?subject <property> ?object . FILTER (isIRI(?object)) } LIMIT <limit> } BIND(REPLACE(STR(?object), \"(.*[/#]).*\", \"$1\") AS ?nspace)} GROUP BY ?nspace ORDER BY DESC(?nsCount)"
    ),
